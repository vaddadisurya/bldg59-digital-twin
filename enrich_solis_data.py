"""
SOLIS DIGITAL TWIN — DATA ENRICHMENT PATCH
============================================
Instead of re-downloading the full 2GB dataset, this script:
1. Loads the existing solis_digital_twin_jan2020.csv (75 cols, 2976 rows)
...
Run: python enrich_solis_data.py
"""
...
for i in ['001', '002', '003', '004']:
    ...
    if speed_col in df.columns:
        ...
        base_temp = 58.0
        speed_effect = -(df[speed_col] - 75) * 0.05  # higher speed = slightly lower temp
        noise = np.random.normal(0, 0.3, len(df))
        df[f'rtu_{i}_sa_temp'] = base_temp + speed_effect + noise
        print(f"  Created rtu_{i}_sa_temp (mean: {df[f'rtu_{i}_sa_temp'].mean():.1f}°F)")
...
legionella_window = (df.index.day >= 15) & (df.index.day <= 17) & (df.index.hour >= 2) & (df.index.hour <= 6)
...
print(f"  Created pump_vibration_mms (start: {df['pump_vibration_mms'].iloc[0]:.1f}, end: {df['pump_vibration_mms'].iloc[-1]:.1f})")

# --- 2g. Chilled Water Temps ---
df['chw_supply_temp'] = 44.0 + np.random.normal(0, 0.5, len(df))  # ~44°F supply
df['chw_return_temp'] = 56.0 + (df['air_temp_set_1'] - 10) * 0.2 + np.random.normal(0, 0.8, len(df))
...
print(f"  Legionella risk alerts: {legionella_count} intervals ({legionella_count/len(df)*100:.1f}%)")
...
df['pump_rul_days'] = ((VIBRATION_FAILURE_THRESHOLD - df['pump_vibration_mms']) / (avg_slope * 96 + 0.001)).clip(lower=0, upper=365)
print(f"  pump_rul_days: start={df['pump_rul_days'].iloc[96]:.0f}, end={df['pump_rul_days'].iloc[-1]:.1f}")
...
print("=" * 60)
groups = {
    'Energy': [c for c in df.columns if any(x in c for x in ['mels', 'lig_s', 'hvac_n', 'hvac_s', 'total_energy', 'hvac_pct', 'hvac_energy'])],
    'RTU Flow/Speed': [c for c in df.columns if 'fltrd_sa_flow' in c or ('vfd_spd' in c and 'volatility' not in c)],
    'RTU Temps (synth)': [c for c in df.columns if any(c == f'rtu_{i}_{t}' for i in ['001','002','003','004'] for t in ['sa_temp','ra_temp'])],
    'RTU Derived': [c for c in df.columns if 'efficiency' in c or 'delta_t' in c or 'volatility' in c],
    'Zone Temps (synth)': [c for c in df.columns if c.endswith('_temp') and c.startswith('zone_')],
    ...
    'Hot Water (synth)': [c for c in df.columns if any(x in c for x in ['hp_hws', 'hw_temp', 'legionella'])],
    'Chilled Water (synth)': [c for c in df.columns if 'chw_' in c],
    'Weather (real)': [c for c in df.columns if any(x in c for x in ['air_temp_set', 'humidity', 'solar', 'dew_point'])],
    'Anomaly Flags': [c for c in df.columns if any(x in c for x in ['ghost_', 'legionella_risk'])],
}
